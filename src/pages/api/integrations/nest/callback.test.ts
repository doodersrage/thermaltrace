...
const mockExchangeNestCode = vi.fn();
vi.mock("../../../../lib/thermostatOAuth", () => ({
  exchangeNestCode: (...a: unknown[]) => mockExchangeNestCode(...a),
}));
...
const mockFetchNestSnapshotDetailed = vi.fn();
vi.mock("../../../../lib/thermostatCorrelation", () => ({
  fetchNestSnapshotDetailed: (...a: unknown[]) => mockFetchNestSnapshotDetailed(...a),
}));
...
function makeContext(options: {
  ...
  const url = new URL("https://example.com/api/integrations/nest/callback");
...
beforeEach(() => {
  ...
  mockGetRuntimeEnv.mockReset().mockImplementation((key: string) => {
    if (key === "NEST_CLIENT_ID") return "nest-client";
    if (key === "NEST_CLIENT_SECRET") return "nest-secret";
    return undefined;
  });
  mockBuildSiteUrl.mockReset().mockReturnValue("https://example.com");
  mockExchangeNestCode.mockReset().mockResolvedValue({
    accessToken: "access",
    refreshToken: "refresh",
    expiresAtMs: Date.now() + 3600_000,
  });
  mockSaveConnection.mockReset().mockResolvedValue({ error: null });
  mockFetchNestSnapshotDetailed.mockReset().mockResolvedValue({ ok: true });
});

describe("GET /api/integrations/nest/callback", () => {
  ...
  it("redirects with not_configured when Nest credentials are missing", async () => {
  ...
  it("redirects with exchange_failed when token exchange fails", async () => {
    mockExchangeNestCode.mockResolvedValue(null);
    const { GET } = await import("./callback");
    const context = makeContext({
      search: { code: "abc", state: "state1" },
      cookies: { thermostat_oauth_state: "state1" },
  ...
  it("redirects with sdm_api_disabled when the Nest probe reports that error", async () => {
    mockFetchNestSnapshotDetailed.mockResolvedValue({
      ok: false,
      errorCode: "sdm_api_disabled",
    });
    const { GET } = await import("./callback");
    const context = makeContext({
      search: { code: "abc", state: "state1" },
  ...
  it("saves the connection and redirects on success", async () => {
    ...
    expect(mockExchangeNestCode).toHaveBeenCalledWith(
      "nest-client",
      "nest-secret",
      "abc",
