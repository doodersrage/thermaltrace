...
const mockBuildEcobeeAuthorizeUrl = vi.fn();
vi.mock("../../../../lib/thermostatOAuth", () => ({
  buildEcobeeAuthorizeUrl: (...a: unknown[]) => mockBuildEcobeeAuthorizeUrl(...a),
}));
...
function makeContext(): APIContext {
  const url = new URL("https://example.com/api/integrations/ecobee/connect");
...
beforeEach(() => {
  ...
  mockBuildEcobeeAuthorizeUrl.mockReset().mockReturnValue(
    "https://ecobee.example/authorize?state=abc",
  );
});

describe("GET /api/integrations/ecobee/connect", () => {
  ...
  it("redirects with not_configured when Ecobee OAuth is not configured", async () => {
    mockBuildEcobeeAuthorizeUrl.mockReturnValue(null);
    const { GET } = await import("./connect");
    const context = makeContext();
  ...
  it("sets the OAuth state cookie and redirects to Ecobee authorize", async () => {
    const { GET } = await import("./connect");
    const context = makeContext();
    ...
    expect(mockBuildEcobeeAuthorizeUrl).toHaveBeenCalledWith(
      expect.any(String),
