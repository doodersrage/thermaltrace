...
import type { BatterySample } from "../lib/batteryTrend";

export type BatteryTrendSeries = {
  label: string;
...
export default function BatteryTrendChart({
  ...
  const usable = useMemo(
    () => series.filter((s) => s.samples.length >= 2).slice(0, 4),
    [series],
  );
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const pcts = usable.flatMap((s) => s.samples.map((sample) => sample.pct));
      const min = Math.max(0, Math.min(...pcts, 0) - 5);
      const max = Math.min(100, Math.max(...pcts, 100) + 5);
      ...
      const yFor = (pct: number) => pad.top + innerH - ((pct - min) / range) * innerH;
      ...
      for (let i = 0; i <= 4; i++) {
        ...
        g.textAlign = "right";
        g.fillText(`${val.toFixed(0)}%`, pad.left - 6, y + 3);
      }
      ...
      if (hover && plotBoundsRef.current) {
        ...
        g.beginPath();
        g.arc(hx, yFor(hover.pct), 4.5, 0, Math.PI * 2);
        g.fill();
      }
    ...
    <>
      ...
      >
        ...
        >
          <canvas
            ref={canvasRef}
            class="w-full history-chart-canvas history-chart-canvas--battery"
            role="img"
            aria-label={title}
            onPointerMove={(e) => {
            ...
            >
              ...
              <ul class="history-chart-tooltip-list">
                <li>
                  ...
                  <span class="history-chart-tooltip-value">{hover.pct.toFixed(0)}%</span>
                </li>
              </ul>
            </div>
          )}
